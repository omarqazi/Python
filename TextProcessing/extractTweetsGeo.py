...
def extractTweets():
  logging.basicConfig(filename='./extractTweetsGeo_'+ str(todayDate[0]) + '.log',level=logging.DEBUG)
  company_list = ['amazon','apple','cisco','dropbox','ebay','facebook','foursquare',\
                  'google','ibm','intel','microsoft', 'qualcomm','twitter']
  count = 0
  tweet_count = 0
  fieldnames=["userid", "lon", "lat", "company", "tweet", "time"]
  for company in company_list:
        output_file = open('./' + company +'/' + company + '_coord_datetime.csv', 'w')
  		  ...
  		  data = json.loads(json_data)
  		    ...
  		    for item in data:
  			      ...
  			      tweet = tweet.replace('\\t',"")                            
                              ...
                              writer.writerow({'userid' : item['user']['id'], 'lon' : item['coordinates']['coordinates'][0], 'lat' : item['coordinates']['coordinates'][1] ,'company' : company, 'tweet' : tweet.encode('utf-8').strip(), 'time' : formatted_date })
